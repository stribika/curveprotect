...
int main(void) {
    ...
    if (verify(buf1, buf2) == 0) fail("fastrandombytes() cryptographically dangerous");

    fastrandombytes(buf1, sizeof buf1);
    fastrandombytes(buf2, sizeof buf2);
    if (verify(buf1, buf2) == 0) fail("fastrandombytes() cryptographically dangerous");

    run(buf1, sizeof buf1);
    run(buf2, sizeof buf2);
    if (verify(buf1, buf2) == 0) fail("fastrandombytes() cryptographically dangerous across fork()");

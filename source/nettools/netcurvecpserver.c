...
#define USAGE "\
netcurvecpserver: how to use:\n\
netcurvecpserver:   -q (optional): no error messages\n\
netcurvecpserver:   -Q (optional): print error messages (default)\n\
netcurvecpserver:   -v (optional): print extra information\n\
netcurvecpserver:   -c n (optional): allow at most n clients at once (default 100)\n\
netcurvecpserver:   sname: server's name\n\
netcurvecpserver:   keydir: use this public-key directory\n\
netcurvecpserver:   ip: server's IP address\n\
netcurvecpserver:   port: server's UDP port\n\
netcurvecpserver:   ext: server's extension\n\
netcurvecpserver:   prog: run this server\n\
"

void die_usage(const char *s)
{
  if (s) die_4(100,USAGE,"netcurvecpserver: fatal: ",s,"\n");
  die_1(100,USAGE);
}
...
void die_fatal(const char *trouble,const char *d,const char *fn)
{
  ...
  if (d) {
    if (fn) die_9(111,"netcurvecpserver: fatal: ",trouble," ",d,"/",fn,": ",e_str(errno),"\n");
    die_7(111,"netcurvecpserver: fatal: ",trouble," ",d,": ",e_str(errno),"\n");
  }
  die_5(111,"netcurvecpserver: fatal: ",trouble,": ",e_str(errno),"\n");
}
...
int main(int argc,char **argv)
{
  ...
  if (!strtoip(serverip,*++argv)) die_usage("ip must be an IP address");
  ...
  fastrandombytes((void *) activeclients,maxactiveclients * sizeof(struct activeclient));
  for (i = 0;i < maxactiveclients;++i) {
    ...
    activeclients[i].sentnonce = fastrandommod(281474976710656LL);
  }
  ...
  udpfdtype = xsocket_type(serverip);
  udpfd = xsocket_udp(udpfdtype);
  if (udpfd == -1) die_fatal("unable to create socket",0,0);
  if (xsocket_bind(udpfd,udpfdtype,serverip,serverport,0) == -1) die_fatal("unable to bind socket",0,0);

  fastrandombytes(minutekey,sizeof minutekey);
  fastrandombytes(lastminutekey,sizeof lastminutekey);
  ...
  for (;;) {
    ...
    if (timeout <= 0) {
      ...
      fastrandombytes(packet,sizeof packet);
      fastrandombytes(packetip,sizeof packetip);
      fastrandombytes(packetport,sizeof packetport);
      fastrandombytes(clientshorttermpk,sizeof clientshorttermpk);
      fastrandombytes(clientshortserverlong,sizeof clientshortserverlong);
      fastrandombytes(nonce,sizeof nonce);
      fastrandombytes(text,sizeof text);
      fastrandombytes(childbuf,sizeof childbuf);
      fastrandombytes(childmessage,sizeof childmessage);
      fastrandombytes(servershorttermpk,sizeof servershorttermpk);
      fastrandombytes(servershorttermsk,sizeof servershorttermsk);
    }
	...
	xsocket_send(udpfd,udpfdtype,packet,200,packetip,packetport,0);
      }
	...
	if (activeclients[i].child == 0) {
	  ...
	  /* XXX: set up environment variables */
          ...
          if (setenv("TCPREMOTEIP",  iptostr(0, packetip), 1) == -1) die_fatal("unable to set environment variable $TCPREMOTEIP",0,0);
	...
	byte_copy(activeclients[i].clientip,16,packetip);
	...
	for (k = 0;k < childbuflen;++k) {
	  ...
	  if (r == 16 * (unsigned long long) activeclients[i].message[0]) {
	    ...
	    xsocket_send(udpfd,udpfdtype,packet,r + 64,activeclients[i].clientip,activeclients[i].clientport,0);
	    activeclients[i].messagelen = 0;
	  }
	}
	break;
	...
	fastrandombytes((void *) &activeclients[numactiveclients],sizeof(struct activeclient));
      } while (0);
    }
  }
}
